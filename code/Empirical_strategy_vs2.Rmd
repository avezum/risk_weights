...
$RW_{b,t}=\gamma_0 IRB_{b,t}+\gamma_1 \overline{RW}_{-b,c,t}+\gamma_2Barth_{c,t}+\alpha_{b}+\alpha_{t}+\varepsilon_{b,t}$ \newline


$RW_{b,t}$ is the risk-weight of exposures of bank $b$ at time $t$. For the years before IRB approval $RW_{b,t}$ takes the SA risk-weight. After approval we replace this variable by the IRB risk-weight. The first instrument is $IRB_{b,t}$ which is a dummy variable tha takes 1 for the periods after bank $b$ is allowed to calculate risk-weights using IRB models. The second instrument, $\overline{RW}_{-b,c,t}$ is the average risk-weight across all banks headquarted in country $c$, excluding bank $b$. $Barth_{c,t}$ is a vector of indices from Barth dataset on bank regulation. $\alpha_{b}$ and $\alpha_{t}$ are bank and time fixed effects.\newline

**Hypothesis** *$\beta<0$: risk-weight arbitrage leads to competitive advantages.*

# Empirical strategy

$IRB_{b,t}$ instruments for the time variation in risk-weights as the approval date is exogenous to the bank. This instrument allow us to estimate the exogenous impact of changes in risk-weight on lending. However, it does not inform us about differences in supervisory standard. For this purpose, we include $\overline{RW}_{c,t}$ and $Barth_{c,t}$. Both measures try to capture the exogenous cross-section of the supervisory leniency/standard. Note, $\overline{RW}_{c,t}$ fails as a proxy for supervisory stardard in the extreme case that systematically banks within the same jurisdiction have the same risk preference.\newline


Following our discussion, we could also try alternative measures. After approval, $RW_{b,t}$ can be calculated as the weighted average by exposure between SA and IRB risk-weights, as the effect should be stronger for banks with a larger share of their corporate portfolio under IRB (larger probability of loan being allocated in the IRB portfolio). Instead of a binary variable $IRB_{b,t}$ could take the initial share of the corporate portfolio that was approved to be calculated using IRB. This alteration would give us some cross-section variation, but it only make sense if we expect that the amount approved was constrained by the supervisory authority and not a bank's choice. We could replace $\overline{RW}_{c,t}$, by $\min{RW}_{c,t}$, that is, the minimum risk-weight observed among all banks in the same jurisdiction, as the measure of supervisory leniency. Lastly, $RW_{b,t}$ can be risk-sensitive. Limiting our analysis to loans where we have external rating information, we can estimate the effect of differences in risk-weight within a specific credit risk cluster (AAA, BB+, etc...). This modification helps us to rule out the possibility that the effect that we estimate is not driven by differences in portfolio risk (intended effect).\newline   
